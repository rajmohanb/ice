...
handle app_create_alloc_req_with_credentials(handle h_authresp)
{
    ...
    status = stun_attr_create(STUN_ATTR_MESSAGE_INTEGRITY, &(h_attr[2]));
    if (status != STUN_OK)
        printf ("stun_attr_create() returned error %d\n", status);
    else
        printf ("stun_attr_create() succeeded\n");
...
void print_hmac_digest_sum(unsigned char* md) {
    int i;
    for(i=0; i <20; i++) {
            printf("%02x",md[i]);
    }
...
/** hadr coded for now */
void app_get_hmac_key(u_char *hmac_key)
{
    u_char raw_material[] = "rajmohab@yahoo.com:viagenie.ca:SandB0x1#";

    platform_md5((unsigned char*) raw_material, strlen(raw_material), hmac_key);
}

void app_verify_server_credentials(u_char *buf, u_int32 bytes, handle h_rcvdmg)
{
    u_char hmac_key[16];
    u_char hmac_digest[20];

    app_get_hmac_key(hmac_key);

    platform_hmac_sha(hmac_key, 16, buf, bytes, hmac_digest);

    print_hmac_digest_sum(hmac_digest);
    printf ("\n");
}
...
int main (int argc, char *argv[])
{
    ...
    if (class_type == STUN_ERROR_RESP)
    {
        ...
        app_verify_server_credentials(buf, bytes, h_rcvdmsg);
