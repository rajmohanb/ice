...
void app_timer_expiry_cb (void *timer_id, void *arg)
{
    ...
    platform_socket_sendto(sock, 
                ...
                AF_INET, DEMO_AGENT_TIMER_PORT, g_localip);
#if 0
    /** 
...
void ice_agent_get_local_ipv4_address (char *address, int addrlen)
{
    ...
    for (ifa = ifAddrStruct; ifa != NULL; ifa = ifa->ifa_next)
    {
        ...
        { // check it is IP4
            ...
            {
                strncpy(address, addressBuffer, addrlen);
            }
...
int32_t ice_agent_parse_user_args(int argc, char *argv[])
{
    int len;
    ...
    while(argc)
    {
        ...
        if ((len == 2) && (strncmp(argv[argc-1], "-a", 2) == 0))
        {
            g_use_aggressive_nomination = true;
        }
        else if ((len == 2) && (strncmp(argv[argc-1], "-s", 2) == 0))
        {
            strcpy(g_stunip, argv[argc]);
        }
        else if ((len == 3) && (strncmp(argv[argc-1], "-ll", 3) == 0))
        {
            g_loglevel = atoi(argv[argc]);
        }
...
int main (int argc, char *argv[])
{
    ...
    if (argc < 3)
    {
        mb_ice_warrior_usage();
        return 0;
    }

    if (ice_agent_parse_user_args(argc, argv) != 0)
    {
        mb_ice_warrior_usage();
        return 0;
    }
    ...
    ice_agent_get_local_ipv4_address(g_localip, 128);
    printf("Local IP Address: %s\n", g_localip);

    /** print the configuration info for the user's benefit */
    if (g_use_aggressive_nomination == true)
        printf("Using Aggressive nomination\n");
    else
